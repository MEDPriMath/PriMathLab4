...
public class JacobiRotation {
    ...
    public static <TMatrix extends Matrix<TMatrix>> Vector calculate(TMatrix matrix, double epsilon) {
        TMatrix a = matrix;

        MatrixJacobiStat stat;
        while ((stat = analyzeMatrix(a)).nonDiagonalSum > epsilon) {
            var phi = phiForRotation(a, stat);
            a = jakobiRotate(a, stat.maxRow, stat.maxColumn, phi);
        }

        Vector eigenValues = new Vector(matrix.rows);
        for (int i = 0; i < matrix.rows; i++) {
            eigenValues.set(i, a.get(i, i));
        }
